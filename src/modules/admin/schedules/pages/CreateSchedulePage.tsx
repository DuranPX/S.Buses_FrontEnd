...
import {
  busesService,
  type Bus,
} from '../../buses/services/busesService';
...
const CreateSchedulePage = () => {
  ...
  const isCiudadano =
    roleName.includes('ciudadano');

  const canCreateSchedules =
    !isCiudadano &&
    can('programaciones', 'escribir');

  const isReadOnly =
    isCiudadano || !canCreateSchedules;
  ...
  const [formData, setFormData] =
    useState<CreateProgramacionDto>({
      ruta_id: '',
      bus_id: '',
      fecha: new Date()
        .toISOString()
    ...
    const fetchData = async () => {
      try {
        ...
        if (busesRes.length > 0) {
          setFormData((prev) => ({
            ...prev,
            bus_id: busesRes[0].id,
          }));
        }
      } catch (err: any) {
        setError(
          'Error al cargar datos necesarios. Verifica la conexión.',
        );
    ...
    >
      ...
      >
        <div>
          ...
          >
            {isReadOnly
              ? 'Visualización de la programación del sistema.'
